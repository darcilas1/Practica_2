...
guardarParqueadero parqueadero = do
    let tempFileName = "parqueadero_temp.txt"
    resultado <- try (writeFile tempFileName (unlines (map mostrarVehiculo parqueadero))) :: IO (Either SomeException ())
    case resultado of
        Right _ -> do
            renameFile tempFileName "parqueadero.txt"
...
listarVehiculos parqueadero = do
    ...
    mapM_ (putStrLn . mostrarVehiculo) (filter (\v -> isNothing (salida v)) parqueadero)
    where
        isNothing Nothing = True
        isNothing _       = False
